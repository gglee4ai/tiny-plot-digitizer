...
키보드 방향키는 선택된 포인트 또는 설정점을 이동합니다. 확대 화면을 클릭하면 선택된 포인트 또는 설정점이 그 위치로 이동합니다. 포인트 탭에서 `[`와 `]`는 이전·다음 포인트를 선택합니다. `Cmd/Ctrl+Z`는 실행 취소, `Shift+Cmd+Z` 또는 `Ctrl+Y`는 다시 실행입니다.
