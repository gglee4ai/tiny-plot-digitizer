...
- 포인트 연속 추가, 선택, 이동과 제거
- 원본 이미지와 확대 화면 클릭을 이용한 0.5픽셀 단위 보정
- 포인트·그룹·좌표설정 변경의 다단계 실행 취소와 다시 실행
- 비정상 종료 시 저장되지 않은 작업의 임시 저장과 복구
- CSV 안의 Tiny Plot Digitizer 설정 헤더와 변환 좌표 저장
...
- `DIGITIZER_PORT`: 로컬 포트. 기본값은 `8837`입니다.
- `DIGITIZER_BROWSER`: `false`로 지정하면 실행 시 브라우저를 자동으로 열지 않습니다.

앱은 로컬 주소 `http://127.0.0.1:8837`에서 실행됩니다.
...
키보드 방향키는 선택된 포인트 또는 설정점을 이동합니다. 확대 화면을 클릭하면 선택된 포인트 또는 설정점이 그 위치로 이동합니다. 포인트 탭에서 `[`와 `]`는 이전·다음 포인트를 선택합니다. `Delete/Backspace`는 포인트 제거 확인창을 열고, `Y`는 제거, `N` 또는 `Esc`는 취소합니다. `Cmd/Ctrl+Z`는 실행 취소, `Shift+Cmd+Z` 또는 `Ctrl+Y`는 다시 실행입니다.
