...
`[`와 `]`는 이전·다음 포인트를 선택합니다. `Shift+Left`와 `Shift+Right`도 같은 기능을 수행합니다. 가운데 되돌리기 버튼은 현재 포인트를 저장 전 위치로 되돌립니다. `저장 전 변경 취소`를 누르면 현재 Figure에서 저장하지 않은 변경만 버립니다. CSV 저장 시 주석과 수정하지 않은 행은 그대로 유지됩니다.
